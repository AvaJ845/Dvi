# Tab 1: Monthly Income Dashboard
    ...
    with tab4:
        if portfolio_type == "ETF Portfolio":
            ...
            fig.update_yaxes(tickprefix='import streamlit as st
import pandas as pd
import numpy as np
...
def main():
    ...
    # Tab 2: Portfolio Analysis
    with tab2:
        st.markdown(f"<h2 class='sub-header'>{portfolio_title} Portfolio Composition</h2>", unsafe_allow_html=True)
        ...
        st.markdown("<h2 class='sub-header'>Dividend Yield Comparison</h2>", unsafe_allow_html=True)
        yield_fig = plot_yield_comparison(portfolio_df, portfolio_title)
        st.plotly_chart(yield_fig, use_container_width=True)
...
if __name__ == "__main__":
    main()
            ...
            stock_row = portfolio_df[portfolio_df['Company'] == selected_growth_stock].iloc[0]
            growth_fig = plot_dividend_growth(stock_row['Ticker'])
            st.plotly_chart(growth_fig, use_container_width=True)import streamlit as st
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
import seaborn as sns
